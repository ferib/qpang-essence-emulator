...
namespace TNL {
	...
	void NetInterface::handleInfoPacket(const Address &address, U8 packetType, BitStream *stream)
	{
		...
		case PacketType::FirstValidInfoPacketId: {
			...
			U32 token = computeClientIdentityToken(address, clientNonce);

			PacketStream pingResponse;

			//Assuming response is packet id + 1
			pingResponse.write(U8(PacketType::PingResponse));
		...
		case PacketType::ConnectChallengeRequest: {
			...
			if (clientIdentityToken == computeClientIdentityToken(address, clientNonce))
			{
			}

			PacketStream challengeResponse;
			challengeResponse.write(U8(ConnectChallengeResponse));

			break;
		}
		default: {
			printf("Unhandled info packet.. \n");
			break;
		}
	...
	void NetInterface::handleConnectChallengeRequest(const Address &addr, BitStream *stream)
	{
		...
		Nonce clientNonce;
		PacketStream p{};
		p.setBuffer(stream->getBuffer(), stream->getBufferSize());
		U8 unk_01;
		U64 unk_02;
		p.readBits(8, &unk_01);
		p.readBits(64, &unk_02);
		clientNonce.read(stream);
	
		bool wantsKeyExchange = stream->readFlag();
		bool wantsCertificate;
		stream->readBits(64, &wantsCertificate);
